...
<div class="card p-4" style="max-width:640px;">
    <form method="POST" action="{{ route('admin.alunos.update', $aluno->id) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nome_completo" class="form-control" value="{{ old('nome_completo', $aluno->nome_completo) }}" required>
        </div>

        <div class="row g-3 mb-3">
            <div class="col-8">
                <label class="form-label fw-semibold">Cidade <span class="text-danger">*</span></label>
                <input type="text" name="cidade" class="form-control" value="{{ old('cidade', $aluno->cidade) }}" required>
            </div>
            <div class="col-4">
                ...
                <select name="estado" class="form-select" required>
                    ...
                    @foreach($estados as $uf)
                        <option value="{{ $uf }}" {{ old('estado', $aluno->estado) === $uf ? 'selected' : '' }}>{{ $uf }}</option>
                    @endforeach
                </select>
            </div>
        </div>
